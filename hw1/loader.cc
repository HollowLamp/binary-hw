  ...
  if(n < 0) {
    ...
    for(i = 0; i < nsyms; i++) {
      if(bfd_symtab[i]->flags & BSF_FUNCTION || bfd_symtab[i]->flags & BSF_OBJECT) {
        ...
        bool is_weak = (bfd_symtab[i]->flags & BSF_WEAK) != 0;
        bool found = false;
        
        Symbol::SymbolType sym_type;
        if (bfd_symtab[i]->flags & BSF_FUNCTION) {
          sym_type = Symbol::SYM_TYPE_FUNC;
        } else {
          sym_type = Symbol::SYM_TYPE_DATA;
        }

        for (auto &existing_sym : bin->symbols) {
            if (existing_sym.name == bfd_symtab[i]->name) {
                found = true;
                if (!is_weak && existing_sym.is_weak) {
                    existing_sym.addr = bfd_asymbol_value(bfd_symtab[i]);
                }
                break;
            }
        }
...
load_dynsym_bfd(bfd *bfd_h, Binary *bin)
{
  ...
  if(n < 0) {
    ...
    for(i = 0; i < nsyms; i++) {
      if(bfd_dynsym[i]->flags & BSF_FUNCTION || bfd_dynsym[i]->flags & BSF_OBJECT ) {

        bool is_weak = (bfd_dynsym[i]->flags & BSF_WEAK) != 0;
        bool found = false;
        ...
        for (auto &existing_sym : bin->symbols) {
            if (existing_sym.name == bfd_dynsym[i]->name) {
                found = true;
                if (!is_weak && existing_sym.is_weak) {
                    existing_sym.addr = bfd_asymbol_value(bfd_dynsym[i]);
                }
                break;
            }
        }
